...
function updateBarangList() {
    ...
    barangList.forEach((item, index) => {
        barangHTML += `<p>${index + 1}. ${item.nama} - RM${item.harga}</p>`;
    });
...
function buatStruk() {
    ...
    barangList.forEach((item, index) => {
        ctx.fillText(`${index + 1}. ${item.nama} - RM${item.harga}`, 20, y);
    ...
    // Total dan admin
    ctx.fillText(`Total: RM${totalHarga}`, 20, y);
    y += 20;
    ctx.fillText(`Fee: RM${hargaAdmin}`, 20, y);
    y += 20;
    ctx.fillText(`Total Keseluruhan: RM${totalKeseluruhan}`, 20, y);
